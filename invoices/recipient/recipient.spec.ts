...
describe("Recipient", () => {
    it("should create an individual recipient", () => {
        const billingResult = Paypal.create({ email: "billing@company.com" }).unwrap();
        ...
        expect(recipient.email.equals(Email.create("info@company.com").unwrap())).toBe(true);
        expect(recipient.name).toBe("John Doe");
        expect(recipient.taxId).toBe("TAX123456");
        expect(recipient.type).toBe("INDIVIDUAL");
        expect(recipient.taxResidenceCountry.equals(Country.create({ code: "US" }).unwrap())).toBe(true);
