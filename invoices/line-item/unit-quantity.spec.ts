...
describe("UnitQuantity", () => {
    it("should create a unit quantity", () => {
        const quantity = UnitQuantity.create(Numeric.create('5')).unwrap();
        expect(quantity.value.equals(Numeric.create('5'))).toBe(true);
    });

    it.each(['-1', '0', '-100', '-5'])("should not create quantity that is not positive: %p", (invalidQuantity) => {
        const result = UnitQuantity.create(Numeric.create(invalidQuantity));
